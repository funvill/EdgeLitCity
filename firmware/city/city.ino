...
#define BUILDING_COUNT 13
#define BUILDING_WALL_COUNT   BUILDING_COUNT * 2 
const byte CITY_WALLS_OFFSET[BUILDING_WALL_COUNT+2] = {
     0,  3, // 0 
    ...
    byte buildingColorsRandomStart[BUILDING_COUNT] ;
...
void setup() {
  ...
  for(byte offset = 0; offset < BUILDING_COUNT ; offset++) {
    buildingColorsRandomStart[offset] = random8() ; 
  }
...
SimplePatternList gPatterns = { PaletteColors, PaletteColorsRow /*, SolidColor , rainbow, rainbowWithGlitter, confetti, sinelon, juggle, bpm */ };
...
void SetBuilding( const byte offsetBuilding, const CRGB color ) {
  ...
  for(byte offsetLed = 0 ; offsetLed < LED_PER_BUILDING ; offsetLed ++ ) {         
    leds[CITY_WALLS_OFFSET[offsetBuilding*2+0] + offsetLed ] = color; 
    leds[CITY_WALLS_OFFSET[offsetBuilding*2+1] + offsetLed ] = color; 
  }
}
...
void RandomColor() {  
  for(byte offset = 0; offset < BUILDING_COUNT ; offset++) {
    buildingColors[offset] = buildingColorsRandomStart[offset] + gHue ; 
  }  
}
...
void PaletteColorsRow() {
  ...
  for(byte offsetRow = 0; offsetRow < COUNT_ROWS ; offsetRow++) {
    paletteIndex = ((0xFF / COUNT_ROWS) * offsetRow) + gHue ; 
    
    switch( offsetRow ) {
      case 0 : 
        SetBuilding( 0, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        break ; 
      case 1 :  
        SetBuilding( 1, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding( 2, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding( 3, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        break ; 
      case 2 : 
        SetBuilding( 4, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding( 5, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding( 6, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding( 7, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding( 8, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        break ; 
      case 3 : 
        SetBuilding( 9, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding(10, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        SetBuilding(11, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));        
        break ; 
      case 4 : 
        SetBuilding(12, ColorFromPalette( currentPalette, paletteIndex, BRIGHTNESS, currentBlending));
        break ; 
    } // End of switch 
  }  // End of offsetRow for loop 
}
